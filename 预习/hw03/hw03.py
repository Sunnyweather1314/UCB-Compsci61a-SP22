...
def count_coins(change):
    ...
    def count(amount , coin):
        if amount <= 0 or len(coin) == 0 :
            ...
            return count(amount-coin[0],coin) + count(amount,coin[1:])
    
    return count(change,[25,10,5,1])
...
def move_stack(n, start, end):
    ...
    # create a dictionary for the whole game plate
    # the number shows the plates on the pole from top to bottom
    # larger number represents larger plate
    '''
    pos = dict()
    pos[start]=[i for i in range(1,n+1)]
    mid = [i  for i in range(1,4) if i != start and i != end][0]
    ...
    if n ==1 :
        ...
        print("Move top diskfrom rod",start,"to rod",end)
...
def make_anonymous_factorial():
    ...
    True
    """
    #reference website: https://stackoverflow.com/questions/15401376/python-lambda-function-to-calculate-factorial-of-a-number
