...
def product(n, term):
    ...
    # HOF + list comprehension
    return functools.reduce(mul,[term(i) for i in range(1,n+1)])
...
def accumulate(merger, base, n, term):
    ...
    # HOF + list comprehension
    return functools.reduce(merger,[base]+[term(i) for i in range(1,n+1)])
    
    #recursive
    if n==0:
        return base
    else:
        return merger(accumulate(merger, base, n-1, term),term(n))
