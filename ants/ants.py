...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        posit = 0 #Initial position 
        while True:
            if (posit >= self.min_range) and (posit <= self.max_range):
                if place_to_check.is_hive == False:
                    if random_bee(place_to_check.bees) != None:
                        return random_bee(place_to_check.bees)
                    else: 
                        place_to_check = place_to_check.entrance
                        posit += 1
                if place_to_check.is_hive == True:
                    return None
            else:
                if place_to_check.is_hive == True:
                    return None
...
class ShortThrower(ThrowerAnt):
    ...
    max_range = 3 # min_range left as it is. max_range is overridden.
...
class LongThrower(ThrowerAnt):
    ...
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
    min_range = 5 # max_range left as it is. min_range is overridden.
    # END Problem 4
...
class HungryAnt(Ant):
    ...
    time_to_chew = 3
    food_cost = 4
    def __init__(self, chew_timer = 0):
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        for b in self.place.bees[:]:
            Ant.reduce_health(b, self.damage) 
# END Problem 9
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        if not gamestate.on_stage:

            gamestate.on_stage = True
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gamestate -- The GameState, used to access game state information.
        """
        print("Debug:"+self.place.exit.name)
        destination = self.place.exit
        # Extra credit: Special handling for bee direction

        if self.slowed == 0 or gamestate.time % 2 == 0:
            if self.scared is True:
                destination = self.place.exit
                # self.place = destination
            elif self.scared is False:
                destination = self.place.exit
                # self.place = destination
            elif self.scared <= 2:# and gamestate.time % 2 == 0:
            ...
            if self.slowed == 0:
                ...
                self.slowed-=1
        elif self.slowed > 0 and gamestate.time % 2 == 1:
            self.slowed -=1
            destination = self.place
            # print("Debug:slow1",self.slowed)
        else:
            self.slowed -=1
        # print("Debug:scare",self.scared)
        # print("Debug:skow",self.slowed)
