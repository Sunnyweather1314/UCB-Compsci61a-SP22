...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    score = [dice() for i in range(num_rolls)]
    if sum([r==1 for r in score]):
        return 1
    else:
        return sum(score)
...
def hefty_hogs(player_score, opponent_score):
    ...
    while opponent_score>0:
        func_num=opponent_score%10
        player_score = digit_fn(func_num)(player_score)
        opponent_score //= 10
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:   
        ...
        add0 = take_turn(num_roll, score0, score1 ,dice,goal)
        score0 += add0
        ...
        add1 = take_turn(num_roll, score1, score0 ,dice,goal)
        score1 += add1
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if leader == last_leader or leader == None:
        ...
        message = "Player " + str(leader) + " takes the lead by " + str(diff)
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    def input(*args):
        result=sum([original_function(*args) for i in range(total_samples)])/total_samples
        return result
    return input
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    result = [make_averaged(roll_dice, total_samples)(i,dice) for i in range(1,11)]
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    add_hefty = hefty_hogs(score, opponent_score)
    add_pile = hog_pile(score+add_hefty, opponent_score)
    if add_hefty + add_pile >= threshold or add_pile > 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
