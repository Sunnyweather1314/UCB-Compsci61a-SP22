...
def largest_factor(n):
    ...
    i = n-2
    if n % 2 ==0:#test whether it is an even number or an odd.
        return n/2#if it is even, we can directly devide it by 2 to get largest factor
    else:
        while i > 0:
            if n % i == 0:
                return i
            i -= 2#-=2 to get only possible odd factors since an odd number can only have odd factors.
