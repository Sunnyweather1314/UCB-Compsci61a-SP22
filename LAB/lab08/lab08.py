...
def merge(incr_a, incr_b):
    ...
    if next_a is None:
        ...
        yield next_b
        yield merge([next_a]+list(iter_a),list(iter_b))
    elif next_a==next_b:
        yield next_b
        yield merge(list(iter_a),list(iter_b))
    else:
        yield next_a
        yield merge(list(iter_a),[next_b]+list(iter_b))
...
def deep_len(lnk):
    ...
    if lnk is Link.empty:
        return 0
    elif isinstance(lnk,int) or (isinstance(lnk.first,int) and lnk.rest is Link.empty):
        return 1
    else:
...
def add_d_leaves(t, v):
    ...
    def tracker(t,v,d):
        if t.is_leaf():
            t.branches += [Tree(v) for _ in range(d)]
        else:
            for b in t.branches:
                tracker(b,v,d+1)
            t.branches +=[Tree(v) for _ in range(d)]
...
def insert_into_all(item, nested_list):
    ...
    #             insert_into_all(item, i)
    #     nested_list.insert(0,item)
    # return nested_list
