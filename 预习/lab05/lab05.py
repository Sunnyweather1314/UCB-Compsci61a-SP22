...
def flatten(s):
    #refernece website: https://likegeeks.com/python-flattening-lists/
    ...
    if len(s)==1:
        ...
        flat=[flatten(s[0])].append([flatten(s[1:])])
    else:
        flat=[s[0]].append(flatten(s[1:]))
    return flat
...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    if sqrt((lat-get_lat(city_a))^2+(lon-get_lon(city_a))^2) > sqrt((lat-get_lat(city_b))^2+(lon-get_lon(city_b))^2):
        return city_b
    else:
        return city_a
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        t = tree(t,leaves)
        return t
    return [sprout_leaves(b, leaves) for b in branches(t)]
