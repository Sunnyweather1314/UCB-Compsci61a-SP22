...
def choose(paragraphs, select, k):
    ...
    try:
        return list(filter(select,paragraphs))[k]
    except IndexError:
        return ''
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def search(para):
        processed = split(lower(remove_punctuation(para)))
        bools=list(map(lambda word: word in processed,topic))
...
def accuracy(typed, reference):
    ...
    if len(reference_words)!=0 and len(typed_words)==0:
        return 0.0
    elif len(reference_words)==0 and len(typed_words)==0:
        return 100.0 
    elif len(reference_words)>=len(typed_words):
        return 100*check(typed_words,reference_words[0:minlen])/minlen
    else:
        return 100*check(typed_words[0:minlen],reference_words)/maxlen
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        diff=[diff_function(typed_word,words,limit) for words in word_list]
        
        if min(diff)<=limit:
            #return word_list[diff.index(min(diff))]
            return min(word_list,key=lambda x: diff_function(typed_word,x,limit))
        return typed_word
    # END PROBLEM 5
...
def sphinx_swaps(start, goal, limit):
    ...
    if len(start)==0 :
        return len(goal)
    #We should return a number greater than limit
    elif limit==-1:
        return 0
    elif len(goal)==0:
        return len(start)
...
def minimum_mewtations(start, goal, limit):
    ...
    if len(goal)==0:
        ...
        sub = minimum_mewtations(start[1:],goal[1:],limit-1)+1
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    grade_d={"id": user_id,"progress":progress}
    upload(grade_d)
    return progress
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    def time_process(time_list):
        start=[0]+time_list[0:-1]
        end=time_list
        return list(map(operator.sub,end,start))[1:]
    
    times=list(map(time_process, times_per_player))
...
def fastest_words(match):
    ...
    for j in word_indices:
        times=[match["times"][i][j] for i in player_indices]
        #print(times)
        highest_index=times.index(min(times))
        #print(highest_index)
        fast_list[highest_index].append(match["words"][j])
